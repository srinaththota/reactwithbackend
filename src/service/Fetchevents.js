...
componentDidMount(){
  ...
  const events =  axios.get('http://localhost:8080/fetchjson').then(response =>{
    ...
    selectEventHandler=(id,homeName,awayName,sport,country)=>{
      this.setState({
        eventSelected:id,
                ...
                return(
                  <div className="container">
                    ...
                    {this.state.events.map(event=>{
                      ...
                      sport={event.sport}
                      status={event.state}
                      clicked={()=>this.selectEventHandler(event.id,event.homeName,event.awayName,event.sport,event.country)}
                      />
